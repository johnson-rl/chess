jQuery(function($) {
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      loadVideo('eu2yodfadf')
    }
  })

  Object.values(videoList).forEach((video)=>{
    $('#select-video').append(`<option value="${video.chapter}">${video.title}</option>`)
  })

  $('#select-video').change(()=>{
    let selected = $('#select-video option:selected').val()
    console.log(selected)
    $.ajax({
      type: 'GET',
      url: `/api/chapter/${selected}`,
      success: (data) => {
        newEvents = data.sort((a, b)=>{return a.timestamp - b.timestamp})
        console.log(newEvents)
  ...
  function handleTimeChange (t) {
    ...
    if (events.length) {
      ...
      if (activeEventId != event.id) {
        console.table([event], ['type', 'timestamp', 'move', 'pgn', 'fen'])
        // do stuff
  ...
  function loadVideo(hash){
    _wq.push({ id: hash, onReady: function(video) {
